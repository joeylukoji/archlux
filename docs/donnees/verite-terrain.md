...
| Étiquettes | `light.simulateur.SimulateurExact` — **une forme fermée** |
| Modèle | `light.base.SubstitutDense`, perceptron 3 couches, poids `numpy` |
| Cible apprise | le **résidu** `SimulateurExact − SubstitutAnalytique` |
...
`mae_analytique = 6,4007` de `resultats/j4_gradient.md` mesure exactement cela :
une régression réussie sur une fonction déterministe, sans bruit.
...
- **aucun mur** (`murs=()`) et **aucune ouverture** (`ouvertures=()`) — les jetons
  ...
  le corpus livré, et le WWR de `SimulateurExact` reste à sa valeur par défaut
