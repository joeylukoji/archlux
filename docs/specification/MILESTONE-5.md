...
def test_couverture_empirique():
    ...
    verites = [ORACLE.evaluer(p, CTX) for p in JEU_TEST]  # SimulateurExact (Radiance)
...
- [ ] 800 – 1 200 évaluations de l'oracle gelé (`SimulateurExact`) pour la calibration
      (Radiance)
...
- [ ] Évaluation de l'oracle gelé, calcul de l'écart (`SimulateurExact` ; (Radiance))
...
def test_derive_bornee():
    r = mesurer_derive(FW, RESEAU, ORACLE, PLANS, CTX)  # ORACLE = SimulateurExact (Radiance)
