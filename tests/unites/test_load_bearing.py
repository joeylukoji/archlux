...
def test_rooms_on_each_side_of_a_full_wall() -> None:
    ...
    sides = deduire_ordre(plan, structure=Structure((_FULL,))).porteurs
    assert set(sides) == {
...
def test_a_room_crossing_the_wall_is_sent_to_the_side_of_its_centre() -> None:
    ...
    sides = {s.room: s.side for s in deduire_ordre(plan, structure=Structure((_FULL,))).porteurs}
...
def test_a_room_beyond_the_end_of_a_partial_wall_stays_beyond_it() -> None:
    ...
    sides = {
        s.room: (s.side, s.bound) for s in deduire_ordre(plan, Structure((_PARTIAL,))).porteurs
    }
...
def test_without_structure_the_order_has_no_wall_sides() -> None:
    assert deduire_ordre(_plan(_room("a", 0, 0, 10, 6))).porteurs == ()
...
def test_legalize_never_crosses_a_load_bearing_wall(objective: object) -> None:
    ctx = _ctx(_FULL)
    plan = _plan(_room("a", 0, 0, 6, 3), _room("c", 0, 3, 6, 3), _room("b", 6, 0, 4, 6))
    result = archlux.legalize(plan, ctx, objective=objective)  # type: ignore[arg-type]
