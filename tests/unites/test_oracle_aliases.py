"""Former names of ``OracleSplitFlux`` stay available, deprecated, until 1.0.0.
...
def test_package_aliases_warn_and_return_the_same_class(old: str) -> None:
    ...
    assert legacy is OracleSplitFlux
...
def test_the_from_import_form_still_works() -> None:
    ...
    assert SimulateurExact is OracleSplitFlux
...
def test_old_names_are_not_advertised() -> None:
    ...
    assert "OracleSplitFlux" in light.__all__
